...
On a Linux-based bare-metal or virtual machine workload, the ThreatStryker sensor agents are deployed as a linux binary.

## ThreatStryker Sensor Agents

Install a docker runtime on the Linux host. Refer to the [Prerequisites for the Sensor Agents](/docs/architecture#threatstryker-sensor-containers) for minimum supported platforms.

* Copy the following shell script and save as `install_deepfence.sh`
```bash
#!/bin/bash

# MGMT_CONSOLE_URL: Example: threatstryker.customer.com or 65.65.65.65
...
# "1" to disable, "" to enable
export DF_DISABLE_TRAFFIC_ANALYSIS=""
export DF_DISABLE_FILE_MON=""
export DF_DISABLE_PROC_MON=""
export DF_DISABLE_LOCAL_TRAFFIC_FILTER=""
...
if [[ "$OS_ID" == "amzn" || "$OS_ID" == "centos" ]]; then
    ...
    yum -y install logrotate jq curl ipset
    if [[ "$?" != "0" ]]; then
    ...
    apt-get -y install logrotate jq curl ipset
    if [[ "$?" != "0" ]]; then
        echo "Failed to install logrotate"
        exit 1
    fi
...
# "1" to disable, "" to enable
Environment="DF_DISABLE_TRAFFIC_ANALYSIS=$DF_DISABLE_TRAFFIC_ANALYSIS"
Environment="DF_DISABLE_FILE_MON=$DF_DISABLE_FILE_MON"
Environment="DF_DISABLE_PROC_MON=$DF_DISABLE_PROC_MON"
Environment="DF_DISABLE_LOCAL_TRAFFIC_FILTER=$DF_DISABLE_LOCAL_TRAFFIC_FILTER"
