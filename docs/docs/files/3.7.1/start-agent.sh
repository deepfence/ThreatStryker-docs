...
start_agent() {
  docker run -dit --cpus=".5" --ulimit core=0 --name=deepfence-agent --restart on-failure --pid=host --net=host --uts=host --privileged=true -v /sys/kernel/debug:/sys/kernel/debug:rw -v /var/log/fenced -v /var/run/docker.sock:/var/run/docker.sock -v /var/lib/docker/:/fenced/mnt/host/var/lib/docker/:rw -v /:/fenced/mnt/host/:ro -e DF_FIM_ON="$FIM_ON" -e DF_TRAFFIC_ANALYSIS_ON="$TRAFFIC_ANALYSIS_ON" -e DF_ENABLE_PROCESS_REPORT="true" -e DF_ENABLE_CONNECTIONS_REPORT="true" -e INSTANCE_ID_SUFFIX="$INSTANCE_ID_SUFFIX" -e DF_PKT_CAPTURE_PERCENTAGE="$CAPTURE_PERCENTAGE" -e USER_DEFINED_TAGS="$USER_DEFINED_TAGS" -e DF_PKT_CAPTURE_SNAP_LENGTH="$SNAP_LENGTH" -e DF_CAPTURE_INTF="any" -e MGMT_CONSOLE_URL="$MGMT_CONSOLE_URL" -e MGMT_CONSOLE_PORT="$MGMT_CONSOLE_PORT" -e SCOPE_HOSTNAME="$DF_HOSTNAME" -e DEEPFENCE_KEY="$DEEPFENCE_KEY" -e DF_TRAFFIC_ANALYSIS_PROCESSES="$TRAFFIC_ANALYSIS_PROCESSES" -e DF_TRAFFIC_ANALYSIS_MODE="$TRAFFIC_ANALYSIS_MODE" ${IMAGE_REPOSITORY}/deepfence_agent:"${DF_IMG_TAG:-3.7.1}"
}
